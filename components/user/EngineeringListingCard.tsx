...
const EngineeringListingCard: React.FC<EngineeringCardProps> = ({
  ...
  return (
    <Link className="" href={`/engineering/${id}`}>
      <div className="relative w-[280px] min-w-[280px] rounded-lg border border-gray-400 bg-white shadow-sm transition xs:w-[330px] sm:w-auto">
        {featured ? (
          <div className="absolute right-2 top-4 rounded bg-primary px-1 text-xs font-bold">
            <span>Featured</span>
          </div>
        ) : null}
        <div className="flex h-[200px] w-full items-center justify-center border-b border-gray-400 p-1">
          {isImage && image ? (
            <BaseImage
